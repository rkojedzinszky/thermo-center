...
from tastypie.authorization import (
    ...
    Authorization as RWAuthorization,
)
...
class SensorResource(ReadonlyPkeyResource):
    valid = fields.BooleanField(
        null=True, readonly=True, help_text="Recent update status"
    )
    ...
    thsensor = fields.ForeignKey(
        "center.restapi.THSensorResource", "", readonly=True, null=True
    )
    ...
    def dehydrate(self, bundle):
        ...
        if bundle._cache:
            ...
            if bundle.data["valid"] == False:
                bundle.data["sensor_resync"] = (
                    SensorResyncResourceInstance.get_resource_uri(bundle.obj)
                )
...
class SensorResyncResource(ReadonlyPkeyResource):
...
class ConfigureSensorTaskResource(ReadonlyPkeyResource):
    ...
    def obj_create(self, bundle, **kwargs):
        if "sensor_name" in bundle.data:
            sensor_id = (
                set(range(1, 128)) - set([s.pk for s in Sensor.objects.all()])
            ).pop()
...
def _receiver_handletask_real(instance):
    channel = grpc.insecure_channel(
        "{}:{}".format(settings.RECEIVER_HOST, settings.RECEIVER_PORT)
    )
